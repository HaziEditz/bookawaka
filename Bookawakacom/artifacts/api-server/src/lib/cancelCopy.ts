/**
 * Keep in sync with INVT/lib/cancelFairness.cjs bookingTimeCancelRules + self-serve lock.
 * Dispatch is the money source of truth; this is passenger-facing copy only.
 */
...
export function selfServeCancelAllowed(status: string | null | undefined): boolean {
  const s = String(status || "")
    ...
    .replace(/[_-]+/g, " ");
  if (!s) return true;
  if (ARRIVED_LOCKED.has(s) || s === "ontrip") return false;
  return true;
}
...
export function companyPhoneFromRecord(rec: Record<string, unknown> | null | undefined): string {
  if (!rec) return "";
  const keys = [
    "phone",
    "contactPhone",
